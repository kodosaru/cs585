...
void trackTemplate(Point2f& point, Mat& patch, Mat& image, Mat& displayImage, int pointNo, int searchRadius)
{
    ...
    // Update patch if old and new correlate
    roi=Rect(maxLoc.x, maxLoc.y, patch.cols, patch.rows);
    Mat newPatch = croppedImage(roi);
    int histSize = 256;
    float range[] = { 0, 255 };
    const float *ranges[] = { range };
    MatND patchHist,newPatchHist;
    calcHist( &patch, 1, 0, Mat(), patchHist, 1, &histSize, ranges, true, false );
    calcHist( &newPatch, 1, 0, Mat(), newPatchHist, 1, &histSize, ranges, true, false );
    double histResult = compareHist(patchHist, newPatchHist, CV_COMP_CORREL);
    //printf("Correlation between old and new patches equals %f\n",histResult);
    if(histResult>0.9)
    {
        cout<<"Patch update"<<endl;
        patch = newPatch;
    }
    else
    {
    ...
    rectangle(displayImage, maxLoc, Point( maxLoc.x + patch.cols , maxLoc.y + patch.rows ), Scalar::all(255), 2, 8, 0 );
